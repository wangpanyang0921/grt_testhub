...
TestHub 是一个**企业级全栈智能测试管理平台**，深度融合 AI 技术与测试工程实践，覆盖从需求分析到测试执行、报告生成的完整测试生命周期。平台采用 Django + Vue3 技术栈，提供现代化的用户界面和丰富的功能特性。

### 🎯 核心价值
- **AI 驱动**: 智能化需求分析、测试用例生成与评审，显著提升测试设计效率
- **全栈覆盖**: 支持 API 测试、Web UI 自动化、Android APP 自动化等多维度测试
- **企业级特性**: 完善的权限管理、评审流程、定时任务、通知机制
- **开箱即用**: 丰富的数据工厂工具集，满足各类测试数据准备需求

## ✨ 核心特性

### 🤖 AI 智能化能力
- **AI 需求分析**: 
  - 自动解析需求文档（PDF/Word/TXT/Markdown），智能提取业务需求
  - 支持需求分级（功能/性能/安全/可用性/接口需求）和优先级划分
  - 自动生成需求编号、验收标准、预计工时
- **智能测试用例生成**: 
  - 基于需求自动生成测试用例，支持功能/集成/API/UI/性能/安全等多种测试类型
  - 完整的用例生命周期：生成 → 评审 → 修订 → 采纳 → 归档
  - 支持 AI 自动评审和人工评审双模式
  - 一键导出 Markdown 格式测试用例
- **智能助手**: 集成 Dify AI 助手，支持多会话管理和测试咨询
- **多模型支持**: 
  - 支持 DeepSeek、通义千问、硅基流动、OpenAI、Anthropic、Google Gemini 等多种 AI 模型
  - 统一的模型配置管理，支持连接测试和验证
- **AI 智能模式（Web 自动化）**: 
  ...
  - 智能任务规划和步骤自动生成
- **Prompt 配置**: 支持自定义 AI 提示词模板，灵活控制生成策略
...
### 📋 测试用例管理
- **完整的用例生命周期管理**: 创建、编辑、版本控制、归档、状态流转
- **灵活的用例组织**: 支持项目、版本、模块、标签等多维度分类
- **详细的用例步骤**: 支持步骤化用例设计，包含前置条件、操作步骤、预期结果
- **优先级与类型**: 支持 P0/P1/P2/P3 优先级，功能/集成/API/UI/性能/安全等测试类型
- **附件和评论**: 支持用例附件上传和团队协作评论
- **批量操作**: 支持用例批量导入导出、批量状态变更
...
- **评审流程管理**: 
  ...
  - 灵活的评审分配机制，支持指派多个评审人
  - 评审截止时间设置和进度跟踪
- **评审状态跟踪**: 待评审、评审中、已通过、已拒绝、已取消等状态管理
- **评审意见记录**: 
  - 支持整体意见、用例意见、步骤意见等多层级反馈
  - 评审意见可标记为已解决/未解决
  - 历史评审记录完整追溯
- **评审模板**: 
  - 可自定义评审检查清单和默认评审人
...
- **项目和集合管理**: 
  ...
  - 项目状态跟踪（未开始/进行中/已结束）
- **请求管理**: 
  - 支持 GET/POST/PUT/DELETE/PATCH/HEAD/OPTIONS 等多种 HTTP 方法
  - 完整的请求配置：URL、Headers、Params、Body、认证信息
  - 前置/后置脚本支持，实现复杂业务逻辑
  - 断言规则配置，支持多维度结果校验
- **环境变量**: 
  - 全局和局部环境变量管理，支持变量替换
  - 多环境切换，快速适配不同测试环境
- **测试套件**: 
  - 批量执行 API 请求，支持断言和执行顺序配置
  - 套件内请求启用/禁用控制
  - 批量导入导出功能
- **请求历史**: 完整的请求执行历史记录和结果追踪
- **定时任务**: 
  - 支持 Cron 表达式、固定间隔、单次执行
  - 邮件/Webhook/企业微信/钉钉/飞书多种通知方式
  - 定时任务执行日志和状态监控
- **测试报告**: 自动生成 Allure 测试报告，支持历史报告对比

### 🖥️ UI 自动化测试（Web）
- **双引擎支持**: 
  - 支持 Selenium 和 Playwright 两种自动化引擎
  - 自动检测系统浏览器和驱动环境
  - 一键安装和更新浏览器驱动
- **元素管理**: 
  - 元素库管理，支持 12 种定位策略（ID、XPath、CSS、name、class、tag、text、placeholder、role、label、title、test-id）
  - 元素分类和标签管理
  - 元素使用统计，优化元素复用
- **页面对象模式（POM）**: 
  - 完整的 POM 设计模式支持
...
- **测试脚本**: 
  - 可视化脚本编辑器，支持步骤录制和回放
  - 丰富的动作库：点击、输入、等待、断言、截图等
  - 脚本版本管理和历史回溯
- **测试套件**: 
  - 批量执行测试脚本，支持多浏览器并行执行（Chrome/Firefox/Edge）
  - 套件内脚本顺序配置和依赖管理
  - 执行环境变量注入
- **执行记录**: 
  - 详细的执行日志、截图、视频录制
...
- **AI 智能模式**:
  ...
  - 智能任务规划和步骤自动生成
  - AI 执行记录和报告生成
- **统一通知配置**: 企业微信、钉钉、飞书等多种 Webhook 机器人统一管理
...
- **设备管理**: 
  - 支持本地模拟器、远程模拟器、真实设备
  - 设备资源池管理，自动发现设备
  - 设备规格信息存储（RAM、CPU、分辨率等）
- **设备锁定**: 
  - 多用户环境下的设备锁定机制，避免资源冲突
  - 锁定超时自动释放（默认8小时）
  - ADB 集成，自动连接远程设备
- **元素管理**: 
  - 支持图片元素、坐标元素、区域元素三种定位方式
  - 多分辨率适配配置
  - 元素标签管理和使用统计
- **组件化编排**: 
  - 基础组件定义和自定义组件组合
...
- **UI Flow**: 
  - JSON 格式的 UI 流程编排
  - 支持 10+ Airtest 动作（点击、滑动、输入、等待、断言等）
  - 变量管理：支持 global/local/outputs 作用域，{{variable}} 语法
- **测试执行**: 
  - Celery 异步执行 + pytest + Allure 报告生成
  - AirtestBase + UiFlowRunner + AppTestExecutor 完整执行引擎
  - 实时执行进度、步骤统计、通过率计算
- **开发体验**: 
  - Monaco Editor 集成，支持 JSON 语法高亮
  - 图片拖拽上传和预览
  - 执行记录自动刷新，实时进度展示
...
### 🏭 数据工厂（51+ 实用工具）
一站式测试数据准备和转换工具集，支持在 API 测试和 UI 测试中直接引用。

- **字符工具**（9个）: 去除空格换行、字符串替换、转义反转义、字数统计、文本对比、正则测试、大小写转换、字符串格式化
- **编码工具**（12个）: Base64编解码、URL编解码、JWT解码、时间戳转换、进制转换、Unicode/ASCII转换、颜色值转换、条形码/二维码生成、图片Base64转换
- **随机工具**（6个）: 随机整数/浮点数、随机字符串、UUID生成、随机布尔值、随机列表元素
- **加密工具**（8个）: MD5/SHA1/SHA256/SHA512哈希、AES加密解密、HMAC签名
- **测试数据**（4个）: 中文姓名、中国手机号、中国邮箱、中国地址生成
- **JSON工具**（8个）: JSON格式化（树形展示）、JSON压缩、JSON校验、JSONPath查询、JSON对比、JSON转XML/YAML/CSV
- **Crontab工具**（4个）: 生成/解析Crontab表达式、获取下次执行时间、验证表达式

**增强特性**:
- **标签系统**: 支持多标签管理，可在接口测试和UI测试中引用带标签的数据
- **使用记录**: 工具使用历史记录和统计，快速复用
