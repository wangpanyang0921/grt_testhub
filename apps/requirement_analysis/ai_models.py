...
class AIModelService:
    ...
    async def _get_matched_template_content(requirement_text: str) -> str:
        ...
        try:
            ...
            # 添加调试日志
            logger.info(f"模板匹配结果: 测试点={len(test_points)}个, 测试场景={len(test_scenarios)}个")
            logger.info(f"需求文本长度: {len(requirement_text)}字符")
            logger.info(f"需求文本前500字符: {requirement_text[:500]}...")
            
            # 记录匹配的模板详情
            if test_points:
                logger.info(f"匹配的测试点: {test_points}")
            if test_scenarios:
                logger.info(f"匹配的测试场景: {test_scenarios}")
